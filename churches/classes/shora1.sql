...
(ifnull((select sum(`t`.`amount`) 
            from transactions` `t` where ((`t`.`operation` = 'credit') and (`t`.`account_id` = `bc`.`id`))),0) 
    - ifnull((select sum(`t`.`amount`) from `transactions` `t` where ((`t`.`operation` = 'debit') and (`t`.`account_id` = `bc`.`id`))),0)) AS `Balance` from ``.`bank_client` `bc`
