<?php 
    if(isset($_POST['ename'])){
        ...
        $edate       =  $_POST['edate'];
        $eid        =  $_POST['eid'];
        $sql = $db->query("UPDATE  event SET eventName = '$ename', eventLocation = '$elocation', eventDate = '$edate', eventTime = '$etime', eventStatus = 'Unreached' WHERE eventId='$eid'");
        $selectEventSql = $db -> query("SELECT * FROM event WHERE eventId = '$eid' LIMIT 1")or die (mysqli_error());
        $rowevent = mysqli_fetch_array($selectEventSql);
        ?>
            <div style="margin-left: 3px;" class="uk-grid uk-grid-medium" data-uk-grid-margin>
                <div class="uk-width-large-4-6">
                    <div class="md-card">
                        ...
                        <div class="md-card-content uk-grid uk-grid-medium" data-uk-grid-margin">
                            ...
                            <div class="uk-width-large-1-2">
                                <button class="md-btn md-btn-success md-fab" onclick="editevent(editId=<?php echo $eid;?>)">Edit</button>
                                <button class="md-btn md-btn-info md-fab" onclick="cover(editId=<?php echo $eid;?>)">cover</button>
                            </div>
                        </div>
                        ...
                        <div class="md-card-content">
                            <table class="uk-table dataTable no-footer">
                                ...
                                <?php
                                    ...
                                    $selectPass = $db ->query("SELECT * FROM eventTicket WHERE eventId = '$eid'");
