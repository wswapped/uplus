...
<?php
include 'db.php';
if (isset($_GET['accountNumber']) && isset($_GET['page'])) {
	...
	$transactions="";
	$pushSql =$con->query("SELECT * FROM transactionsview WHERE operation = 'DEBIT' AND status <> 'BALANCE' AND bankCode = '$page' AND accountNumber='$accountNumber'");
$n=0;
while ($row=mysqli_fetch_array($pushSql)) 
{
	...
	elseif($pushStatus=='DECLINED' || $pushStatus=='NO MONEY' || $pushStatus=='NET?'){
		$bg="#f44336";
	}
	else{
		$bg="#000";
	...
	elseif($pullStatus=='DECLINED' || $pushStatus=='Declined' || $pullStatus=='Error sending money.' || $pullStatus=='NET?'){
		$bg2="#f44336";
	}
	else{
		$bg2="#000";
...
<nav class="navbar navbar-default">
  ...
  <div class="collapse navbar-collapse" id="menu">
    <ul class="nav navbar-nav">
      ...
      <li class="active"><a href="javascript:void()"><?php echo $accountNumber;?></a></li>
    </ul>
...
<div class="container">
    ...
    color: #fff; color: #fff; font-size: 20px; background-color: '.$bankcolor.'; height: 100px;">
						...
						&nbsp;&nbsp;&nbsp;('.$n.') transactions of '.$accountNumber.' in '.$bankName.'
					</div>';
		}else{
		...
		<div class="col-xs-2">
			<div class="table-responsive">
				<table class="table table-hover table-condensed table-striped table-bordered" style="float: left;">
					...
					<?php
						include 'db.php';
						$sql =$con->query("SELECT * FROM transactionsview WHERE bankCode = '$page' AND accountNumber='$accountNumber' GROUP BY actorName ");
						while ($row=mysqli_fetch_array($sql)) {
							echo '<tbody><tr>
						<td><a href="bankClient.php?page='.$page.'&accountNumber='.$row['accountNumber'].'&ClientName='.$row['actorName'].'">'.$row['actorName'].'</a></td>
					</tr>
